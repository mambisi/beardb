...
impl TableIndexBuilder {
    fn create_table_offsets(blocks: &Vec<u8>, index: &Vec<u32>, bloom_filter : Vec<BloomFilter>) -> Vec<BlockOffsetsIndex> {
        ...
        for (i, (start, filter)) in index.iter().zip(bloom_filter).enumerate() {
            ...
            let base_key = Vec::from(&array[BLOCK_ENTRY_HEADER_SIZE..BLOCK_ENTRY_HEADER_SIZE + key_size]);
    ...
    pub(crate) fn new(blocks: &Vec<u8>, index: &Vec<u32>,bloom_filter : Vec<BloomFilter>, key_count: u32) -> Self {
        Self {
            inner: TableIndex {
    ...
    pub(crate) fn finish<'a>(self) -> crate::Result<AlignedVec> {
        let bytes = rkyv::to_bytes::<_, INDEX_BLOCK_SIZE>(&self.inner).map_err(|e| {
...
impl<'a> TableIndexReader<'a>  {
    pub(crate) fn new_from_slice(slice : &'a[u8], opts : Arc<TableOptions>) -> TableIndexReader<'a> {
        let inner = unsafe { rkyv::archived_root::<TableIndex>(slice) };
        Self {
            inner,
            opts
        }
    ...
    pub(crate) fn find_key_block(&self, key : &[u8]) -> Option<BlockIndex> {
        let pos = self.inner.offsets.partition_point(|t| {
            key.ge(t.base_key.as_slice())
        });
