...
impl<W> TableBuilder<W> where W : Write  {
    ...
    pub(crate) fn add<'a>(&mut self, key : &'a [u8], value : &'a [u8]) -> crate::Result<()>{
        if self.should_finish_block(key,value) {
            ...
            self.filters.push(self.opt.policy.create_filter_from_hashes(self.current.key_hashes.as_slice()));
    ...
    fn add_internal<'a>(&mut self, key : &'a [u8], value : &'a [u8]) -> crate::Result<()> {
        ...
        written_bytes += block.data.write((key.len() as u32).to_le_bytes().as_slice())?;
        written_bytes += block.data.write((value.len() as u32).to_le_bytes().as_slice())?;
    ...
    fn should_finish_block<'a>(&mut self, key : &'a [u8], value : &'a [u8]) -> bool {
        ...
        let est_block_size = block.data.len() + BLOCK_ENTRY_HEADER_SIZE + key.len() + value.len() + (block.entry_offsets.len() * 4) + BLOCK_META_SIZE + CHECKSUM_SIZE;
    ...
    pub(crate) fn finish(mut self) -> crate::Result<()>{
        ...
        self.dst.write_all(index_block_size.to_le_bytes().as_slice())?;
